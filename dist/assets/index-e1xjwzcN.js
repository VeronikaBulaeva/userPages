...
<timer-counter></timer-counter>
  ...
  `;sessionStorage.setItem("timeStart",new Date().getTime().toString());document.getElementById("menuIsOpen").onclick=function(){document.getElementById("headerBottomMenu").classList.toggle("is-open")};const b={"/userPages/":{title:"Profile",render:p},"/userPages/map":{title:"Map",render:m},"/userPages/timer":{title:"Timer",render:g}};function n(){let a=b[location.pathname];document.title=a.title,a?(document.title=a.title,app.innerHTML=a.render()):(history.replaceState("","","/userPages/"),n());const s=document.getElementById("TimerNav"),i=document.getElementById("MapNav"),c=document.getElementById("ActivityNav");switch(location.pathname){case"/userPages/timer":s.classList.add("a-focus"),i.classList.remove("a-focus"),c.classList.remove("a-focus");break;case"/userPages/map":i.classList.add("a-focus"),c.classList.remove("a-focus"),s.classList.remove("a-focus");break;case"/userPages/":c.classList.add("a-focus"),s.classList.remove("a-focus"),i.classList.remove("a-focus");break}}window.addEventListener("click",a=>{a.target.matches("[data-link]")&&(a.preventDefault(),history.pushState("","",a.target.href),n())});window.addEventListener("popstate",n);window.addEventListener("DOMContentLoaded",n);
